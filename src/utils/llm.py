...
def call_llm(prompt: Any, model_name: str, model_provider: str, pydantic_model: Type[T], agent_name: Optional[str] = None, max_retries: int = 3, default_factory=None) -> T:
    """
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Log the failed call
            LLMLogger.log_call(prompt=prompt, model_name=model_name, model_provider=model_provider, response=None, agent_name=agent_name, duration_ms=(end_time - start_time) * 1000, success=False, error_message=error_msg)
